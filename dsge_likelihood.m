...
% NOTE: This file over rides the default dynare dsge_likelihood() function
...
%% Penalize contemporaneous cross-correlation in shocks
% This version: August 19, 2016

%% Extract the estimated shocks
gend = DynareOptions.nobs;
data = DynareDataset.data;
data_index = DynareDataset.missing.aindex;

% etahat is a (r*T) matrix of smoothed structural shocks (r>n is the umber of shocks).
[alphahat,etahat,epsilonhat,ahat,SteadyState,trend_coeff,aK,T,R,P,PK,decomp]=DsgeSmoother(xparam1, gend,data,data_index,0); 

%% Calculate the matrix norm as the degree of contemporaneous cross-correlation in the variance-covariance matrix. 

cov(etahat');
V=corrcoef(etahat');
Loss2=norm(V-eye(length(V))); 

%% Triger the penalty (requires DynareOptions.custom_penalty != 0)

if DynareOptions.custom_penalty
    penalty = DynareOptions.custom_penalty;
    fval_original = fval;
    % Assumption: we want the penalty to increase fval 
    % (I think this makes sense because dynare_estimation_1.m tries to minimize fval)
    fval = fval_original + penalty*Loss2; 
    disp(sprintf('fval = %0.5g (fval_original = %0.5g, Loss2 = %0.5g)', fval, fval_original, Loss2));
else
    disp(sprintf('fval = %0.5g (no penalty applied)', fval));
end

% TODO: first check whether DynareOptions.custom_penalty is defined
% TODO: only extract the estimated shocks and calculate the norm if
% custom_penalty != 0. But for testing purposes, I want to do this always.
